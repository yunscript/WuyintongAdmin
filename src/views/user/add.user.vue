...
<div class="app-container">
  ...
  <div class="checkbox margin-bottom10" v-for="(permissionItem, i) in permission">
    <el-checkbox v-model="permissionItem.trade.selectAll" @change="handleAllChange(permissionItem.trade.selectAll, permissionItem.trade.menuLevel)">{{ permissionItem.trade.oneLable}}</el-checkbox>
    <el-row class='ml15'>
      <el-col :span="6" v-for="(items, index) in permissionItem.trade.menuLevel">
        <div class="mt10">
          <el-checkbox v-model="items.selectAll" @change="handleCheckAllChange(permissionItem.trade, items.selectAll, items.checkedCities, items.twoLable)">{{ items.oneLable }}</el-checkbox>
          <div style="margin: 15px 0;"></div>
          <el-checkbox-group v-model="items.checkedCities" @change="handleCheckedCitiesChange(permissionItem.trade, 'selectAll', index, items.twoLable, items.checkedCities)">
  ...
  data() {
    return {
      ...
      permission: [{
        ...
        trade: {
          oneLable: '链上数据查询', //  一级选项
          checkedCities: [],
          selectAll: false,
          menuLevel: [{
            oneLable: '企业基本信息查询', //  一级选项
            twoLable: [], //  二级选项
            checkedCities: [], //  默认选中
            selectAll: false //  一级全选控制按钮
          }, {
        ...
        trade: {
          ...
          menuLevel: [{
            oneLable: '用户管理', //  一级选项
            twoLable: ['修改', '删除', '新增用户', '角色增加', '角色删除'], //  二级选项
            checkedCities: [], //  默认选中
            selectAll: false //  一级全选控制按钮
          }, {
            oneLable: '系统消息',
    ...
    handleAllChange(selectAll, options) {
      if (selectAll) {
        options.forEach((item, i) => {
          console.log('options[i].checkedCities = ' + options[i].checkedCities)
